...
#if 0
boardd.cc 파일은 comma.ai의 오픈파일럿 시스템에서 
차량의 CAN 네트워크와 통신하는 판다 장치를 관리하는 코드를 포함하고 있습니다. 
이 코드는 판다 장치들이 네트워크에 연결되어 있는지 확인하고, 
CAN 메시지를 보내고 받으며, 판다의 상태를 확인하고 업데이트합니다. 
또한, 안전 모드 설정, 시간 동기화, 하드웨어 모니터링 등의 기능을 수행합니다. 
특히, 차량의 점화 상태에 따라 판다의 안전 모드를 설정하고, 
차량이 꺼져 있을 때는 안전 모드를 'NO_OUTPUT'으로 설정하여 판다가 CAN 메시지를 보내지 않게 합니다.

이 파일에는 여러 스레드를 생성하고 관리하는 코드도 포함되어 있으며, 
이 스레드들은 판다 장치로부터 CAN 메시지를 받고, 
차량의 주변 장치 상태를 전송하며, 
판다 장치의 상태를 주기적으로 업데이트하는 역할을 합니다. 
전체 시스템이 정상적으로 작동할 수 있도록 각 스레드는 서로 다른 기능을 수행하면서 동시에 실행됩니다.
#endif
